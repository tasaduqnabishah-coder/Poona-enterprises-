...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Image Area - Completely Clickable to Detail Page */}
      <Link to={`/product/${product.id}`} className="relative aspect-square overflow-hidden rounded-lg bg-slate-50 mb-4 block cursor-pointer">
        <img
      ...
      <div className="flex flex-col flex-grow px-1">
        <div className="flex justify-between items-start mb-2">
          ...
          <p className="font-bold text-slate-900">
            {isRent ? (
              <span className="text-sm font-semibold text-primary">View Rates</span>
            ) : (
              <>₹{product.price}</>
            )}
          </p>
        ...
        {isRent && (
          <div className="mb-4 bg-slate-50 p-2.5 rounded border border-slate-100">
            <p className="text-[9px] text-slate-500 leading-relaxed italic">
              <span className="font-bold text-slate-700 not-italic">Note:</span> Pickup charges apply for short-term. Free return for 6+ months!
            </p>
          </div>
        )}
        
        {/* Click to Details instead of jumping straight to WhatsApp on catalog */}
        <div className="mt-auto pt-2">
          <Link
            to={`/product/${product.id}`}
            className="text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-2 hover:gap-3 transition-all cursor-pointer"
          >
